  ...
  export default function QuestionPaperManagement() {
    ...
    const handleDownloadQuestionPaper = async (questionPaper: QuestionPaper) => {
      try {
        ...
        toast({
          ...
          description: `Opening ${questionPaper.title}`,
        });
      } catch (error) {
        toast({
          title: "Error",
    ...
    const updateQuestionTypeDistribution = (
      ...
      setFormData((prev) => {
        ...
        const currentTotal = dists.reduce((sum, d) => {
        ...
        if (selectedSubjects.length === 0) {
          ...
          const aiRequest = {
            ...
            markCategories.forEach((category) => {
              // Use the original formData.markDistribution for question count, not adjustedMarkDistribution
          ...
          const hasSubjectQuestionTypes = subjectQuestionTypeDistribution && 
            Object.values(subjectQuestionTypeDistribution).some((distributions: any) => 
          ...
          const aiRequest = {
            ...
            questionTypeDistribution: (() => {
              ...
              for (const mark of markCategories) {
                // Use the original markDistribution for question count check
    ...
    const validateQuestionPaperForm = () => {
      ...
      if (selectedSubjects.length > 1) {
        ...
        // Validate question type distribution doesn't exceed mark distribution
        const markCategories = ['oneMark', 'twoMark', 'threeMark', 'fiveMark'] as const;
        for (const category of markCategories) {
          const maxQuestions = formData.markDistribution[category];
          const totalUsed = getQuestionTypeTotal(category);
          
          if (totalUsed > maxQuestions) {
            errors.push(
              `${category} question type distribution (${totalUsed}) exceeds the configured number of questions (${maxQuestions})`
            );
          }
        }
      }
          ...
          <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto">
              ...
              <div className="space-y-6">
                    ...
                    <div className="space-y-6">
                      ...
                      <div className="space-y-4">
                        ...
                        <div className="bg-gray-100 p-3 rounded text-xs">
                          <strong>Debug - Current questionTypeDistribution:</strong>
                          <pre className="mt-1">{JSON.stringify(formData.questionTypeDistribution, null, 2)}</pre>
                          <button 
                            onClick={() => updateQuestionTypeDistribution('oneMark', 'TRUE_FALSE', 10)}
                            className="mt-2 px-2 py-1 bg-blue-500 text-white rounded text-xs"
                          >
                            Test: Set 10 True/False for 1-mark
                          </button>
                        </div>
                ...
                {selectedSubjects.length > 1 && (
                  <div className="space-y-4">
                    ...
                    {/* Debug display for overall subjectDistributions state */}
                    <div className="bg-blue-50 p-3 rounded text-xs">
                      <strong>Debug - Overall subjectDistributions State:</strong>
                      <pre className="mt-1 text-xs overflow-auto max-h-40">
                        {JSON.stringify(subjectDistributions, null, 2)}
                      </pre>
                      <div className="mt-2 flex gap-2">
                        <button 
                          onClick={() => {
                            const newSubjectDistributions = { ...subjectDistributions };
                            selectedSubjects.forEach(subject => {
                              newSubjectDistributions[subject._id] = {
                                ...newSubjectDistributions[subject._id],
                                questionTypeDistribution: {
                                  ...newSubjectDistributions[subject._id]?.questionTypeDistribution,
                                  oneMark: [
                                    { type: 'TRUE_FALSE', questionCount: 3 }
                                  ]
                                }
                              };
                            });
                            setSubjectDistributions(newSubjectDistributions);
                          }}
                          className="px-2 py-1 bg-blue-500 text-white rounded text-xs"
                        >
                          Test: Set 3 True/False for all subjects
                        </button>
                        <button 
                          onClick={() => setSubjectDistributions({})}
                          className="px-2 py-1 bg-red-500 text-white rounded text-xs"
                        >
                          Clear All Subject Distributions
                        </button>
                      </div>
                    </div>
                      ...
                      return (
                        <Card key={subjectData._id} className="p-4">
                          <CardHeader>
                            ...
                            {/* Debug display for subject-specific distribution */}
                            <div className="bg-yellow-50 p-2 rounded text-xs mt-2">
                              <strong>Debug - Subject Distribution State:</strong>
                                ...
                                onClick={() => {
                                  const newDistribution = {
                                    ...
                                    questionTypeDistribution: {
                                      ...
                                      oneMark: [
                                        { type: 'TRUE_FALSE', questionCount: 5 }
                                      ]
                                    }
                                  };
                                  setSubjectDistributions((prev) => ({
                                    ...prev,
                                    [subjectData._id]: newDistribution
                                  }));
                                }}
                                className="mt-1 px-2 py-1 bg-yellow-500 text-white rounded text-xs"
                              >
                                Test: Set 5 True/False for this subject
                              </button>
                            </div>
                          </CardHeader>
            ...
            {currentStep === 3 && (
              <div className="space-y-6">
                ...
                {/* Debug display for payload that will be sent */}
                <div className="bg-green-50 p-4 rounded-lg border-2 border-green-200">
                  <h4 className="text-sm font-bold text-green-800 mb-3">🔍 Debug - Payload Preview</h4>
                  <div className="space-y-3">
                    <div>
                      <strong className="text-xs text-green-700">Form Data (Global):</strong>
                    ...
                    <div>
                      <strong className="text-xs text-green-700">Selected Subjects:</strong>
                    ...
                    <div>
                      <strong className="text-xs text-green-700">Subject Distributions:</strong>
                      <pre className="mt-1 text-xs bg-white p-2 rounded border overflow-auto max-h-40">
                        {JSON.stringify(subjectDistributions, null, 2)}
                      </pre>
                    </div>
                    ...
                    <div>
                      <strong className="text-xs text-green-700">Custom Marks:</strong>
                      <pre className="mt-1 text-xs bg-white p-2 rounded border overflow-auto max-h-32">
                        {JSON.stringify(customMarks, null, 2)}
                      </pre>
                    </div>
                    
                    <div>
                      <strong className="text-xs text-green-700">Uploaded Pattern:</strong>
                      <pre className="mt-1 text-xs bg-white p-2 rounded border overflow-auto max-h-32">
                        {JSON.stringify({
                          fileName: uploadedPattern?.name,
                  ...
                  <div className="mt-4 flex flex-wrap gap-2">
                    <button 
                      onClick={() => {
                        ...
                        console.log("🚀 Final AI Request Payload:", aiRequest);
                        alert("Payload logged to console! Check browser dev tools.");
                      }}
                    ...
                    <button 
                      onClick={() => {
                        console.log("📊 Current State Summary:", {
                          ...
                          customMarks,
                          uploadedPattern: uploadedPattern?.name,
                          uploadedPatternId
                        });
                        alert("State summary logged to console!");
                      }}
                  ...
                  <button 
                    onClick={() => {
                      console.log("🚨 TESTING ACTUAL API CALL LOGIC");
                      console.log("formData.questionTypeDistribution:", formData.questionTypeDistribution);
                      
                      // Test the conversion logic that's used in the actual API call
                      const testConversion = (() => {
                        ...
                        markCategories.forEach((category) => {
                          ...
                          console.log(`Testing ${category}:`, {
                            questionCount,
                            questionTypes,
                            questionTypesLength: questionTypes.length
                          });
                        ...
                        console.log("Final converted object:", converted);
                        return converted;
                      })();
                      
                      console.log("🧪 TEST CONVERSION RESULT:", testConversion);
                      alert("Test conversion logged to console! Check if the conversion logic is working correctly.");
                    }}
                    className="px-3 py-2 bg-red-600 text-white rounded text-xs font-medium hover:bg-red-700"
                  >
                  ...
                  <button 
                    onClick={() => {
                      console.log("🔍 COMPARING MARK DISTRIBUTIONS");
                      console.log("formData.markDistribution:", formData.markDistribution);
                      
                      // Calculate adjustedMarkDistribution like in the actual API call
                      const totalFromQuestions =
                        formData.markDistribution.oneMark * 1 +
                        formData.markDistribution.twoMark * 2 +
                        formData.markDistribution.threeMark * 3 +
                      ...
                      console.log("adjustedMarkDistribution:", adjustedMarkDistribution);
                      console.log("Difference:", {
                        oneMark: adjustedMarkDistribution.oneMark - formData.markDistribution.oneMark,
                        twoMark: adjustedMarkDistribution.twoMark - formData.markDistribution.twoMark,
                        threeMark: adjustedMarkDistribution.threeMark - formData.markDistribution.threeMark,
                        fiveMark: adjustedMarkDistribution.fiveMark - formData.markDistribution.fiveMark
                      });
                      
                      alert("Mark distribution comparison logged to console!");
                    }}
                    className="px-3 py-2 bg-orange-600 text-white rounded text-xs font-medium hover:bg-orange-700"
                  >
                    🔍 Compare Mark Distributions
                  </button>
                  </div>
                </div>
                <div className="space-y-4">
